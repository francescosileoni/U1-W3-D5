...
const whoIsBigger = function (num1, num2) {
  if (num1 > num2) {
    return console.log("il numero piu grande è", num1);
  } else {
    return console.log(" il numero piu grande è", num2);
  }
};
whoIsBigger(78, 35);
...
*/
const deleteOne = function (str, boolean1) {
  if (boolean1 === true) {
...
deleteOne("buongiorno caro", false);
/* ESERCIZIO 5
...
const whatDayIsIt = function () {
  const currentDay = daysNames[now.getDate];
  return currentDay;
};
...
const isTodayMyBirthday = function () {
  const now = new Date();
  const compleanno = "3-9";
  const compleanno1 = new Date(compleanno);
};
// Arrays & Oggetti
...
const newFunction = function () {
  ...
  for (let i = 0; i < tdSelect.length; i++) {
    console.log(tdSelect[i].innerText);
  }
};
/* ESERCIZIO 23
...
const backGround = function () {
  const hrefSelect = document.getElementsByTagName("a");
  for (let i = 0; i < hrefSelect.length; i++) {
    hrefSelect[i].style.backgroundColor = "red";
  }
};
...
const removeList = function () {
  ...
  for (let i = 0; i < liSelect.length; i++) {
    liSelect[i].style.visibility = "hidden";
  }
};
/* ESERCIZIO 26
